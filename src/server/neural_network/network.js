...
class Network {
  constructor(numberOfLayers) {
    ...
    this.layers = numberOfLayers.map((length, index) => {
      ...
      //l = Letra para el numero de variables 
      if (index === 0) {
      ...
      for (var neuron = 0; neuron < this.layers[layer].neurons.length; neuron++) {
        ...
        connectionsValue.map((s, i) => {
          ...
          if (layer === this.layers.length - 1) {
            let p = 0;
            // 'o' es una variable simbolica para determinar el numero de variables de la funcion
            ...
            //pasamos a valores parciales el output
  ...
  calculateDeltasSigmoid(target /*this.mejorResultado*/) {
